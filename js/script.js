(function($) {
    ...
    -------------------------------------------*/
    if ($("#clock").length) {
        $('#clock').countdown('2025/03/29', function(event) {
            var $this = $(this).html(event.strftime(''
            + '<div class="box dias"><div>%D</div> <span>Días</span> </div>'
            + '<div class="box hrs"><div>%H</div> <span>Horas</span> </div>'
            + '<div class="box min"><div>%M</div> <span>Mins</span> </div>'
            + '<div class="box seg"><div>%S</div> <span>Segs</span> </div>'));
        });
    }
